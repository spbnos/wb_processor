...
# Promotion (advertising) audit — read-only live WB API integration
try:
    from api.routes import promotion_audit as promotion_audit_module
    app.include_router(promotion_audit_module.router, prefix="/api")
    logger.info("[main] Promotion audit router registered")
except ImportError as _e:
    logger.warning(f"[main] Promotion audit router not available: {_e}")
